...
class GeneticAlgorithm():
    ...
    def _mutate(self, individual, highest_fitness, avg_fitness):
        ...
        while True:
            random_time_slot1 = np.random.randint(0,len(all_sesi)-1)
            random_time_slot2 = np.random.randint(0,len(all_sesi)-1)
    ...
    def run(self):
        ...
        print ("[%d Answer: '%s']\n [Fitness: %.2f]" % (optimum_epoch, fittest_individual, highest_fitness))
        ...
        print(x,y,z,p,q,r)
        return self.parseJsn(repaired_individu,x,y,z,p,q,r,highest_fitness,optimum_epoch)

    def parseJsn(self, individual,x,y,z,p,q,r,highest_fitness,optimum_epoch):
        result = {
            # "data": [],
            # "skpb": [],
            "violated_constraint":{},
        ...
        result['optimum_epoch'] = optimum_epoch
...
def main():
    ...
    for key, chunks in chunks.items():
        print(key)
        GA = GeneticAlgorithm(10, chunks)
        result = GA.run()
        res[key] = result
        f = open(f'result_{key}.json', "w")
